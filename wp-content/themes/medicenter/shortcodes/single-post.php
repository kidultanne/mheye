...
if((int)$columns==3){
	$output .= '<div class="columns clearfix thd-page-left">';
	$output .= '<ul class="blog clearfix sec-page-body">';
	$post_classes = get_post_class("post");
	$output .= '<li class="single ';
	foreach($post_classes as $key=>$post_class)
		$output .= ($key>0 ? ' ' : '') . $post_class;
	$output .= '">';
				if((int)$columns==3 || (int)$show_post_date_box || (int)$show_post_comments_box)
				{
				$output .= '<div class=" thd-article-head-box clearfix">';
					if((int)$columns==3 || (int)$show_post_date_box)
					{
					$output .= '<div class="thd-article-head">
                        <h2><a href="' . get_permalink() . '" title="' . get_the_title() . '">' . get_the_title() . '</a></h2>
                        <ul class="clearfix">
                            <li>文章来源：<span>北京美和眼科</span></li>
                            <li>发布时间：<span>'. mb_strtoupper(date_i18n("Y-m-j", get_post_time())) .'</span></li>
                            <li>浏览次数：<span>'.getPostViews(get_the_ID()).'</span></li>
                        </ul>
                    </div>';
					}
	...
	vc_map( array(
		...
		"params" => array(
			...
			array(
				...
				"param_name" => "columns",
				"value" => array(__("Type 1", 'medicenter') => 1, __("Type 2", 'medicenter') => 2, __("Type 3", 'medicenter') => 3)
			),
			array(
				"type" => "dropdown",
				"class" => "",
