...
load_theme_textdomain( 'tgmpa', get_template_directory() . '/languages4623' );
load_theme_textdomain( 'js_composer', get_template_directory() . '/languages4623' );
load_theme_textdomain( 'medicenter', get_template_directory() . '/languages4623' );
load_theme_textdomain( 'woocommerce', get_template_directory() . '/languages4623' );
//四亩地
...
mc_get_theme_file("/widgets/widget-scrolling-broadcast.php");
mc_get_theme_file("/widgets/widget-scrolling-cooperation-platform.php");
...
function theme_after_setup_theme()
{
	...
	if(!get_option($themename . "_installed") || !get_option("wpb_js_content_types"))
	{
		$theme_options = array(
			...
			"footer_text_left" => "© Copyright - <a href='http://www.4mudi.com' target='_blank' title='wordpress主题'>四亩地</a> ",
	...
	<body>
		...
		update_option("blogdescription", "有温度的儿童眼科!");
		
		add_option("wpb_js_content_types", array(
			"page",
			"doctors", 
			"medicenter_gallery", 
			)//去掉"features" 2017-04-05 Anne
		);
...
function theme_enqueue_scripts()
{
	...
	global $theme_options;
	//style
	// 四亩地	if(!empty($theme_options["header_font"]))
	// 四亩地		wp_enqueue_style("google-font-header", "//fonts.googleapis.com/css?family=" . urlencode($theme_options["header_font"]) . (!empty($theme_options["header_font_subset"]) ? "&subset=" . implode(",", $theme_options["header_font_subset"]) : ""));
	// 四亩地	else
	// 四亩地		wp_enqueue_style("google-font-droid-sans", "//fonts.googleapis.com/css?family=PT+Sans");
	// 四亩地	if(!empty($theme_options["subheader_font"]))
	// 四亩地		wp_enqueue_style("google-font-subheader", "//fonts.googleapis.com/css?family=" . urlencode($theme_options["subheader_font"]) . (!empty($theme_options["subheader_font_subset"]) ? "&subset=" . implode(",", $theme_options["subheader_font_subset"]) : ""));
	// 四亩地	else
	// 四亩地		wp_enqueue_style("google-font-droid-serif", "//fonts.googleapis.com/css?family=Volkhov:400italic");
	...
	// 四亩地	wp_enqueue_script("google-maps-v3", "//maps.google.com/maps/api/js" . ($theme_options["google_api_code"]!="" ? "?key=" . esc_attr($theme_options["google_api_code"]) : ""), false, array(), false, true);
...
include_once('my-visual-composer.php');
